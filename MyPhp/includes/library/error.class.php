...
class Core_Error
{
	...
	public static function display($sMsg, $iErrCode = null)
	{
		if (CORE_IS_AJAX){
			...
			Core::getLib('module')->setController('error.display');
	 ...
	 */
	public static function set($sKey, $sMsg)
	{
		self::$aErrors[$sKey] = $sMsg;
	...
	public static function get($sKey = '')
	{
        if(empty($sKey))
            ...
            $sStr = (isset(self::$aErrors[$sKey])) ? self::$aErrors[$sKey] : ''; 
            return $sStr;
        }
	}	

    /**
    * convert all error value to string. 
    * for each value will separated by tag <br />  
    * 
    */
	...
	{		
		...
		if ((defined('CORE_LOG_ERROR') && CORE_LOG_ERROR)){
			self::log($sErrMsg, $sFileName, $nLinenum);
		}
		
		if (!CORE_DEBUG && !error_reporting()){
	    	return false;
	    }	    
    	...
    	$sErrMsg = str_replace(DIR, '', $sErrMsg);
    	if ($bNoHtml){
	        ...
	        <tr>
	        	<td colspan="10" align="left" valig="top" style="background-color: ' . $aColors[$nErrNo] . '"><b>' . $aTypes[$nErrNo] . ':&nbsp;' . $sErrMsg . ' - ' . str_replace(DIR, '', $sFileName) . ' (' . $nLinenum . ')</b></td></tr>';  	
    	}    	
    	
   		$aFiles = debug_backtrace();
   		
   		if (preg_match('/mysql_connect\(\)/i', $sErrMsg)){			
			exit($sErrMsg);
			// cant connect to database : too many connections
			// display the no DB error page
			$sFile = file_get_contents(DIR . 'static' . DS . 'nodb.html');
			ob_clean();
			die($sFile);
    	...
    	for ($i=$iStart, $n=sizeof($aFiles); $i<$n; ++$i){
        	...
        	$sFile = str_replace(DIR, '', $aFiles[$i]['file']);
        	
        	if ($bNoHtml){
    			$sErr .= "{$i}\t{$sFile}\t" . (isset($aFiles[$i]['line']) ? $aFiles[$i]['line'] : '') . "\t" . strip_tags(str_replace(DIR, '', $sFuncName)) . "\n";	
    		}else {
        		$sErr .= '<tr><td bgcolor="#DDEEFF" align="right">'.$i.'&nbsp;</td>'.
                 ...
                 '<td style="border-bottom:1px #000 solid; border-left:1px #000 solid;">' . str_replace(DIR, '', $sFuncName) . '</td></tr>';
    		}
    	}  		
	    ...
	    if (CORE_DEBUG && defined('CORE_DEBUG_EXIT')){      	
	    	exit;
	    }
	}	
	 ...
	 */
	public static function log($sMessage, $sFile, $iLine = 0)
	{
		...
		if(!empty($sFile)){
            ...
            $hFile = fopen(DIR . 'file' . DS . 'log' . DS . 'error_log_' . date('d_m_y', time()) . '_' . md5(Core::getVersion()) . '.php', 'a');
        }
        fwrite($hFile, "##\n{$sErrorLog}##\n");
        fclose($hFile);    
	}
...
function e()
{
	$bCliOrAjax = ((PHP_SAPI == 'cli' || (defined('CORE_IS_AJAX') && CORE_IS_AJAX)));
	ob_clean();
	if (!$bCliOrAjax){
		echo '<link rel="stylesheet" type="text/css" href="theme/adminpanel/default/style/default/css/debug.css?v=' . CORE_TIME . '" />';
	}
	define('CORE_MEM_END', memory_get_usage());
	echo Debug::getDetails();
	exit;
}
