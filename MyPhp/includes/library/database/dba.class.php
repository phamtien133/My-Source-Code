<?php
Core::getLibClass('database.interface');
abstract class Database_Dba implements Database_Interface
{
    /**
    ...
    public function limit($iPage, $sLimit = null, $iCnt = null, $bReturn = false, $bCorrectMax = true)
    {
        ...
        if ($bCorrectMax == true){
            $iOffset = ($iCnt === null ? $iPage : Core::getLib('pager')->getOffset($iPage, $sLimit, $iCnt));
     ...
     */
    public function replace($sTable, $aValues = array(), $bEscape = true)
    {
        if (!$aValues){
            $aValues = $this->_aData;
        }
        ...
        $sSql = $this->_replace($sTable, implode(', ', array_keys($aValues)), $sValues);
        $this->sSql = $sSql;
        if ($hRes = $this->query($sSql)){
            return $this->getLastId();
        }
    ...
    public function update($sTable, $aValues = array(), $sCond = null, $bEscape = true)
    {
        ...
        foreach ($aValues as $sCol => $sValue){
            ...
            if(strpos($sValue, '`') !== false) {
				$sReq = '/`(.*)`(\+|-) *([0-9]+)$/';
				preg_match($sReq, $sValue, $aMath);
                if(count($aMath) && !empty($aMath[1])) $var = 1;
			}
            if($var == 1) {
                $sSets .= "{$sCol} {$sCmd} " . ($sValue) . ", ";
            }
            else
    ...
    public function delete($sTable, $sQuery, $iLimit = null)
    {
        ...
        //return $this->query("UPDATE {$sTable} SET trang_thai = 2 WHERE ". $sQuery);        
    }   
     ...
     * Example:
     * <code>
     * Core::getLib('database')->updateCounter('user_count', 'total_friend', 'user_id', 1);
     * </code>
     *
    ...
    {        
        $iCount = $this->select($sCounter)->from(Core::getT($sTable))->where($sField . ' = ' . (int) $iId)->execute('getSlaveField');
        
        $this->update(Core::getT($sTable), array($sCounter => ($bMinus === true ? (($iCount <= 0 ? 0 : $iCount - 1)) : ($iCount + 1))), $sField . ' = ' . (int) $iId);
    }    
    ...
    public function updateCount($sCountTable, $aCountCond, $sCounter, $sUpdateTable, $aUpdateCond)
    {
        $iCount = $this->select('COUNT(*)')
            ->from(Core::getT($sCountTable))
            ->where($aCountCond)
            ->execute('getSlaveField');
            
        $this->update(Core::getT($sUpdateTable), array($sCounter => $iCount), $aUpdateCond);
    }
    ...
    public function searchKeywords($sField, $sStr)
    {
        ...
        if (count($aWords)){
            ...
            foreach ($aWords as $sWord){
                ...
                $sQuery .= $sField . ' LIKE \'%' . Core::getLib('database')->escape($sWord) . '%\' ';
                
                $aLikeWords = $this->getLikeWords($sWord);
                
                foreach ($aLikeWords as $sLikeWord){
                    if (strpos($sQuery, $sLikeWord) === false){
                        $sQuery .= ' OR ' . $sField . ' LIKE \'%' . Core::getLib('database')->escape($sLikeWord) . '%\'' ;
                    }
                }
        ...
        if (!$iIteration){
            return $sField . ' LIKE \'%' . Core::getLib('database')->escape($sStr) . '%\' ';
        }
    ...
    */
     ...
     * @return string Returns the actual SQL query to perform
     */
    protected function _replace($sTable, $sFields, $sValues)
    {
        return 'REPLACE INTO ' . $sTable . ' '.
            '        (' . $sFields . ')'.
            ' VALUES (' . $sValues . ')';            
    }
    ...
    {
        $this->sSql = 'UPDATE ' . $sTable . ' SET ' . $sSets . ' WHERE ' . $sCond;
