<?php
class Database_Driver_Mysql extends Database_Dba
{    
    /**
    ...
    {        
        ...
        if (Core::getParam(array('db', 'slave'))){
            // Get the slave array
            $aServers = Core::getParam(array('db', 'slave_servers'));
    ...
    public function query($sSql, &$hLink = '')
    {
        ...
        $hRes = @($this->_aCmd['mysql_query'] == 'mysqli_query' ? $this->_aCmd['mysql_query']($hLink, $sSql) : $this->_aCmd['mysql_query']($sSql, $hLink)) or Core_Error::errorHandler(E_ERROR, 'SQL error '. $sSql,  $_SERVER['SCRIPT_FILENAME'].'-'.end(get_included_files()), 0);
...
//    {
//        if (!is_dir($sPath)){
//            return 'the_path_you_provided_is_not_a_valid_directory';
//        }
//        
//        if (!Core::getLib('file')->isWritable($sPath, true)){
//            return 'the_path_you_provided_is_not_a_valid_directory';
//        }
//        
//        $sPath = rtrim($sPath, DS) . DS;
//        $sFileName = uniqid() . '.sql';
//        $sZipName = 'sql-backup-' . date('Y-d-m', CORE_TIME) . '-' . uniqid() . '.tar.gz';
//        
//        shell_exec("mysqldump --skip-add-locks --disable-keys --skip-comments -h". Core::getParam(array('db','host')) ." -u". Core::getParam(array('db','user')) ." -p". Core::getParam(array('db','pass')) ." ". Core::getParam(array('db','name')) ." > " . $sPath . $sFileName ."");
//        chdir($sPath);
...
//    
    ...
    protected function _getRow($sSql, $bAssoc, &$hLink)
    {
        ...
        // Get the array
        $aRes =  $this->_aCmd['mysql_fetch_array']($hRes, ($bAssoc ? MYSQLI_ASSOC : MYSQLI_NUM));
