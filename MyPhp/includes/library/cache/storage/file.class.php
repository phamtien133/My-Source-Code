<?php
class Cache_Storage_File extends Cache_Abstract
{
	/**
	...
	{		
		if (is_array($sName)){
			if (CORE_SAFE_MODE || CORE_OPEN_BASE_DIR){
				$sName = str_replace(array('/', DS), '_', $sName[0]) . '_' . $sName[1];
			}else{
				$sNewDirectory = DIR_CACHE . $sName[0];
				if (!is_dir($sNewDirectory)){
					Core::getLib('file')->mkdir($sNewDirectory, true, 0777);				
				}
				$sName = rtrim($sName[0], '/') . DS . $sName[1];
			}
		}
	...
	public function remove($sName = null, $sType = '')
	{
		...
		if ($sName === null){
			foreach ($this->getAll() as $aFile){
				if (file_exists(DIR_CACHE . $aFile['name'])){
					if (is_dir(DIR_CACHE . $aFile['name'])){
						Core::getLib('file')->delete_directory(DIR_CACHE . $aFile['name']);
					}else {
						unlink(DIR_CACHE . $aFile['name']);
					
						$this->removeInfo(DIR_CACHE . $aFile['name']);
					}
				}
			}						
		...
		switch($sType){
			case 'substr':
				$sDir = DIR_CACHE . (is_array($sName) ? rtrim($sName[0], '/') . DS : '');
				if (!CORE_SAFE_MODE && !CORE_OPEN_BASE_DIR && !is_dir($sDir)){
					Core::getLib('file')->mkdir($sDir, true, 0777);
				}
				$aFiles = Core::getLib('file')->getFiles($sDir);
				...
				foreach ($aFiles as $sFile){
					...
					if (is_dir($sDir . $sFile)){
						Core::getLib('file')->delete_directory($sDir . $sFile);
					}else {
						@unlink($sDir . $sFile);
					}
	...
	public function isCached($sId, $iTime = 0)
	{
		...
		if (isset($this->_aName[$sId]) && file_exists($this->_getName($this->_aName[$sId]))){
			if ($iTime && (CORE_TIME - $iTime * 60) > (filemtime($this->_getName($this->_aName[$sId])))){
	...
	public function getCachedFiles($aConds = array(), $sSort, $iPage, $sLimit)
	{
		...
		$aRows = array();
		$iSize = 0;
		$aFiles = Core::getLib('file')->getAllFiles(DIR_CACHE);
		$iLastFile = 0;
		foreach ($aFiles as $sFile){
	...
	private function _getName($sFile)
	{
		...
		return DIR_CACHE . (Core::getParam('core.cache_add_salt') ? md5(Core::getParam('core.salt') . $sFile) : $sFile) . Core::getParam('core.cache_suffix');
	}	
}
