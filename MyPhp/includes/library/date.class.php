...
class Date
{
    ...
    public static function getTimeZone($bDst = true)
    {
        ...
        if ($bDst === false)
        {
            ...
            {
                $sUserOffSet = Core::getParam('core.default_time_zone_offset');
            }
            if (substr($sUserOffSet,0,1) == 'z' && CORE_USE_DATE_TIME)
            {
                $aTZ = Core::getService('core')->getTimeZones();
    ...
    public function daysToDate($iTimeEnd, $iTimeStart = null, $bRound = true)
    {
        ...
        $iYearEnd = (date('Y', Core::getTime()) > date('Y', $iTimeEnd)) ? date('Y', Core::getTime()) : date('Y', $iTimeEnd);
        if (date('m', Core::getTime()) > $iMonthEnd)
        {
            $iYearEnd++;
        }
    ...
    {
        return $this->mktime(0, 0, 0, Core::getTime('m'), (date('d', $this->getWeekEnd()) + 1), Core::getTime('Y'));
    }
    ...
    {
        return $this->mktime(23, 59, 59, Core::getTime('m'), (date('d', $this->getWeekEnd()) + 2), Core::getTime('Y'));
    }
    ...
    {
        if(Core::getTime('m') == Core::getTime('m', $this->getWeekEnd()))
        {
            return $this->mktime(0, 0, 0, Core::getTime('m'), (date('d', $this->getWeekEnd()) - 6), Core::getTime('Y'));
        }
        else
        {
            $iCnt = 7 - date('d', $this->getWeekEnd());
            if(in_array(Core::getTime('n'), array(1,3,5,7,8,10,12)))
            {
                $iDay = 31 - $iCnt;
            }
            elseif(Core::getTime('n') == 2)
            {
                $iDay = 28 - $iCnt;
            }
            else
            ...
            return $this->mktime(0, 0, 0, Core::getTime('m'), $iDay, Core::getTime('Y'));
        }
    }
    ...
    {
        return ($this->mktime(0, 0, 0, Core::getTime('m'), 1, Core::getTime('Y')) - 3500);    
    }
    ...
    public function convertFromGmt($iTime, $sOffset)
    {
                ...
                $aTimeZones = Core::getService('core')->getTimeZones();
                // If PHP 5.3.0 or up
    ...
    {
        $iSeconds = (int) round(abs(CORE_TIME - $iTime));
        ...
        {
            return ' Yesterday' . ', ' . Core::getTime(Core::getParam('core.conver_time_to_string'), $iTime);
        }    
        
        return Core::getTime(Core::getParam(($sDefault === null ? 'core.global_update_time' : $sDefault)), $iTime);
    }
    ...
    {    
        $iCurrentHour = (int) Core::getTime('H');        
    ...
    {            
        if (!Core::getParam('core.identify_dst'))
        {
            return false;
        }
        
        return (Core::getUserBy('dst_check') ? true : false);    
    }
}
