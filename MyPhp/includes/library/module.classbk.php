    ...
    public function getComponent($sClass, $aParams = array(), $sType = 'block', $bTemplateParams = false)
    {
		...
		if($oSession->getArray('session-user', 'id') < 1 || $oSession->getArray('session-user', 'priority_group') == -1)
		{
			...
			else
			{
				if(!empty($_POST))
				{
					echo '<body style="height: 100%;width: 100%;"><script type="text/javascript" src="http://'.str_replace('cms.', '', $_SERVER['HTTP_HOST']).'/styles/web/global/js/jquery-1.9.1.min.js?v='.$config['versionExFile'].'"></script><script src="./styles/web/global/js/main.js?v=2.8" ></script><meta http-equiv="Content-Type" content="text/html; charset=utf-8" />Phiên làm việc bị kết thúc.<br>Vui lòng bấm vào link để đăng nhập, sau đó trở lại và bấm F5.</body>';
				}
				else if($oSession->getArray('session-user', 'priority_group') == -1)
				{
					echo 'No permission';
				}
				else echo 'No User';
			}
			echo '<br><a href="//'.str_replace('cms.', '', $_SERVER['HTTP_HOST']).'/dang_nhap.html?refer='.base64_encode(Core::getParam('core.main_path')).'" target="_blank">Click here to Login..</a>';
			exit;
		}
		// permission - END
        if ($sType == 'ajax' && strpos($sClass, '.') === false){
