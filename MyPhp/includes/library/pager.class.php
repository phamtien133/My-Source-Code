...
class Pager
{
    ...
    {    
        ...
        {
            $this->_aParams = array_merge(Core::getLib('request')->getRequests(), $aParams['aParams']);
        }
        else 
        {
            $this->_aParams = Core::getLib('request')->getRequests();
        }      
    ...
    public function process(&$aRows)
    {
        ...
        Core::getLib('template')->assign('iPagerNextPageCnt', $iNextPage);        
    }
    ...
    private function _getInfo()
    {
        ...
        if ($this->_iPage != 1)
        {
            ...
            Core::getLib('template')->setHeader('<link rel="prev" href="' . $aInfo['prevUrl'] . '" />');
        }        
        ...
        if ($this->_iPagesCount != $this->_iPage)
        {
            ...
            Core::getLib('template')->setHeader('<link rel="next" href="' . $aInfo['nextUrl'] . '" />');
