...
class Request
{
    ...
    {
        $this->_aArgs = $this->_trimData(array_merge($_GET, $_POST, $_FILES, Core::getLib('url')->getParams()));
    }
    ...
    /**
     ...
     {
         return md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null) . 'iysduyt623rts' . $this->getSubstrIp($this->getIp()));
     }
    ...
    {   
        // tạm thời ko enable bản mobile.     
        return false;
        
        static $bFirstCheck = false;
        
        if ($bFirstCheck === false && !Core::getLib('url')->isMobile() && !Core::getLib('session')->get('mobilestatus'))
        {
            $bFirstCheck = true;
            
            $this->getBrowser();
            
            if ($this->_bIsMobile === true && !CORE_IS_AJAX)
            {   
                $sSendWhere = 'mobile';
                
                if ($bRedirect && !empty($sSendWhere))
                {
                    Core::getLib('url')->send($sSendWhere);
                }
                else
                {
                    return true;
        ...
        return Core::getLib('url')->isMobile();        
    }
    ...
    public function getServer($sVar)
    {
        switch($sVar)
        {
            ...
            case 'PHPFOX_SERVER_ID':
                //if (!Core::getParam(array('balancer', 'enabled')))
//                {
                    //if (Core::getParam('core.allow_cdn'))
//                    {
//                        return Core::getLib('cdn')->getServerId();
//                    }
//                    
//                    return 0;
//                }
                $aServers = Core::getParam(array('balancer', 'servers'));
    ...
    */
    ...
    {
        return md5(time() . Core::getParam('core.path') . $this->getIdHash() . Core::getParam('core.path') . rand(1, 1000000));
    }
    ...
    public function send($sUrl, $aPost = array(), $sMethod = 'POST', $sUserAgent = null, $aCookies = null, $bFollow = false)
    {
        ...
        if (extension_loaded('curl') && function_exists('curl_init'))
        {
            ...
            curl_setopt($hCurl, CURLOPT_USERAGENT, ($sUserAgent === null ? "" . 'Bong' . " (" . '1.0'. ")" : $sUserAgent));
     ...
     */
     public function getSubstrIp($sIP, $iLength = null)
     {
         ...
         {
             $iLength = Core::getParam('core.ip_check');
         }
