...
class File
{
	 ...
	 * Example:
	 * <code>
	 * $aMeta = Core::getLib('file')->getMeta('/var/www/sample.jpg');	 
	 * </code>
	...
	public function getMeta($sFileName)
	{
		...
		require_once(DIR_LIB . 'getid3' . DS . 'getid3' . DS . 'getid3.php');
	...
	{				
		...
		{
			return Core::setError('Not a valid file extension.');
		}		
		...
		if (Core::getLib('image')->isImageExtension($this->_aFile['ext']) && !Core::getLib('image')->isImage($this->_aFile['tmp_name']))
		{
            return Core::setError('Not a valid image. We only accept the following file extensions: jpg, png, gif');
		}	
    ...
    public function upload($sFormItem, $sDestination, $sFileName, $bModifyFileName = true, $iPerm = 0644, $buildDir = true, $bCdn = true)
    {
        ...
        {
        	$sFileName = md5($sFileName . time() . uniqid());
        }
        ...
        {
            return Core::setError('Unable to move the file.');
        }
		...
		if (Core::getParam('core.allow_cdn') && $bCdn === true)
		{
			$bReturn = Core::getLib('cdn')->put($sDestination . str_replace('\\', '/', str_replace($sDestination, '', $this->_sDestination) . $sFileName . '.' . $this->_sExt));
	...
	{		
		if ($hFile = @fopen(DIR_CACHE . $sFile, 'w+'))
		{			
			fwrite($hFile, $sData);
			fclose($hFile);		
		}
	...
	{	    
		...
		if (Core::getParam('core.allow_cdn') && $iServerId > 0)
		{
			//$sFile = Core::getLib('cdn')->getUrl(str_replace(PHPFOX_DIR, Core::getParam('core.path'), $sFile), $iServerId);					
			$sFileSize = $sFileSize ? $sFileSize : filesize($sFile);
			$sFile = Core::getLib('cdn')->getUrl(str_replace(DIR, Core::getParam('core.path'), $sFile), $iServerId); 
		}		
	    ...
	    {
	    	$sServer = Core::getLib('request')->getServerUrl($iServerId);
	...
	{
		if (Core::getParam('core.allow_cdn'))
		{
			try 
		    { 
		        Core::getLib('cdn')->remove($sSrc); 
		    } 
	    	catch(Exception $e){}	    	
		}		
    ...
    {    	
        ...
        if (stristr(PHP_OS, "win") || $bForce === true)
        {
            ...
            if (is_dir($sPath))
            {
                 ...
                 */
                $fp = @fopen($sPath . DS . 'win-test.txt', 'w');                
                if (!$fp)
                {
                    if ($bForce === true)
    				{
	...
	{		
		if ($bRecurse === true) {			
			...
			$sParentDirectory = (Core::getLib('server')->isWindows() ? '' : DS);
    ...
    private function _passLimit()
    {
		...
		*/
		...
		{
			//return Phpfox_Core_Error::set('Image dimensions too small');
		}
    	switch ($this->_aFile['error'])
    	{
    		case 1:
                return Core::setError('Upload failed. Server cannot handle files larger then: '.ini_get('upload_max_filesize'));
    			break;
    		default:
			    	...
			    	{
                        return Core::setError('Upload failed. Server cannot handle files ('.$this->filesize($iSize, 0).') larger then: '.$this->filesize($iUploadMaxFileSize));
			    	}    
			    	
			    	if ($this->_iMaxSize !== null && $iSize >= ($this->_iMaxSize * 1048576))
			    	{
                        return Core::setError('Upload failed. Your file ('.$this->filesize($iSize, 0).') is larger then our limit: '.$this->filesize(($this->_iMaxSize * 1048576)));
			    	}
	    			return true;
    			break;
    	}
	...
	{
		if (!CORE_SAFE_MODE && Core::getParam('core.build_file_dir') && !defined('CORE_IS_HOSTED_SCRIPT'))
		{
			$aParts = explode('/', Core::getParam('core.build_format'));
			$this->_sDestination = $sDestination;
	...
	{		
		if (Core::getParam('core.enable_getid3_check'))
		{		
			$aMeta = $this->getMeta($this->_aFile['tmp_name']);
			
			if (!isset($aMeta['mime_type']))
			{
                return Core::setError('Uploaded file is not valid.');
			}
		}		
