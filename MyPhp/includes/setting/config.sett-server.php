<?php

		// Report all errors except E_NOTICE
		//error_reporting(E_ALL & ~E_NOTICE & ~E_WARNING & ~E_STRICT);
		//ini_set('display_errors', 1);
		
$_CONF['db']['driver'] = 'mysqli';
$_CONF['db']['host'] = 'localhost'; 
$_CONF['db']['user'] = 'root';
$_CONF['db']['pass'] = '0e#&2d7eaa145c12fb173f0';
$_CONF['db']['name'] = 'theme_new_2';
$_CONF['db']['prefix'] = 'eratown_';
$_CONF['db']['port'] = '';

$_CONF['db']['slave'] = false;
$_CONF['db']['slave_servers'] = array();

$_CONF['core.dir'] = '/web/admin/';
if(substr($_SERVER["HTTP_HOST"], 0, 4) == 'sup.') 
    $_CONF['core.main_server'] = 'sup.';
else 
    $_CONF['core.main_server'] = 'cms.';
$_CONF['core.ads_server'] = 'ads.vi';
$_CONF['core.pay_server'] = 'pay.';
$_CONF['core.versionExFile'] = '0.9.5';
$_CONF['core.local'] = 0;
$_CONF['core.phpexe'] = 'E:/server/bin/php/php.exe';

$_CONF['core.url_rewrite'] = '1';

$_CONF['core.salt'] = '4adec0e2d7eaa145c12fb173f0cb2db1';

$_CONF['core.cookie_path'] = '/';
$_CONF['core.cookie_domain'] = $_SERVER['HTTP_HOST'];//str_replace('cms.', '', $_SERVER['HTTP_HOST']);
session_set_cookie_params(0, '/', $_CONF['core.cookie_domain']);//'.'.$_CONF['core.cookie_domain']);
// Storage Engine (file, memcache)
$_CONF['core.cache_storage'] = 'memcache';

// Add salt
$_CONF['core.cache_add_salt'] = false;

// Cache suffix (file only)
$_CONF['core.cache_suffix'] = '.php';
$_CONF['core.cache_prefix'] = 'test_';

// Memcache Hosts
$_CONF['core.memcache_hosts'] = array(
    'loccalhost' => array(
        'host' => '127.0.0.1',
        'port' => '11211'
    )
);

// Memcahe persistent
$_CONF['core.memcache_persistent'] = false;

// Should we skip the cache check and display live content
$_CONF['core.cache_skip'] = false;

$_CONF['core.cache_template_file'] = false;

$_CONF['balancer']['enabled'] = true;
$_CONF['balancer']['servers'] = array();
$_CONF['server']['code_generate'] = array(
    'gomart.vn' => array( //tên miền
        'import' =>  array( //loại mã
            'file' => 'import.txt', //tên file
            'length' => 4, //chiều dài chuỗi ký tự số
            'suffix' => '', //phần text phía sau chuổi số
            'prefix' => 'PN' //phần text phía trước chuổi số
        ), 
        'export' => array(
            'file' => 'export.txt',
            'length' => 4,
            'suffix' => '',
            'prefix' => 'PX'
        ),
    ),
    'disieuthi.vn' => array( //tên miền
        'import' =>  array( //loại mã
            'file' => 'import.txt', //tên file
            'length' => 4, //chiều dài chuỗi ký tự số
            'suffix' => '', //phần text phía sau chuổi số
            'prefix' => 'PN' //phần text phía trước chuổi số
        ), 
        'export' => array(
            'file' => 'export.txt',
            'length' => 4,
            'suffix' => '',
            'prefix' => 'PX'
        ),
    )
);
