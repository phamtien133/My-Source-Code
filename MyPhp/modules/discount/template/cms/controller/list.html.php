...
<div class="container product unselectable page-list-vendor" ng-controller="product-ctrl">
    ...
    <div class="title_list">
      ...
      <div class="cl2 cl js-sort-by" data-sort=1>
        <div class="tt or">
          Mã số
        </div>
      ...
      <div class="cl5 cl js-sort-by" data-sort=3>
        <div class="tt or">
          Mã
        </div>
      ...
      <div class="cl7 cl js-sort-by" data-sort=5>
        <div class="tt or">
          S.L đã S.D
        </div>
      ...
      <div class="cl7 cl js-sort-by" data-sort=7>
        <div class="tt or">
          Thời gian S.D gần nhất
        </div>
        <div class="<?php if(isset($sap_xep) && $sap_xep == 6):?>ic ic1 <?php elseif (isset($sap_xep) && $sap_xep == 7):?>ic ic3 <?php endif?> bg js-icon-sort"></div>
        <div class="clear"></div>
        <div class="hv"></div>
      </div>
      <div class="cl8 cl" data-sort=11>
        <div class="tt or">
          Giá trị
        </div>
        <div class="clear"></div>
    ...
    <?php if (count($aData['id']) > 0):?>
    <?php for($i=0;$i<count($aData['id']);$i++):?>
    <div class="r" id="tr_object_<?=$aData['id'][$i]?>">
      <!--div class="cl1 cl">
        <md-checkbox class="ck checkBox" ng-click="checkProduct" ng-model="checkBoxProduct<?=$aData['id'][$i]?>"></md-checkbox>
        <div class="clear"></div>
      </div-->
      ...
      <div class="cl7 cl atv">
        <div class="tt or">
          <?= $aData['total_used'][$i]?>/<?= $aDiscount['times_to_use']?>
        </div>
        <div class="clear"></div>
      </div>
      <div class="cl7 cl atv">
        <div class="tt or">
          <?= $aData['last_time_used'][$i]?>
        </div>
        <div class="clear"></div>
      </div>
      ...
      <div class="cl9 cl">
        <?php if($aData['status'][$i] == 1):?>
        <div class="stic bg ic1 js-activity" id="js-status-object-<?= $aData['id'][$i]?>" data-id="<?= $aData['id'][$i]?>" data-status="1">
          <div class="sp">
            <div class="p">
        ...
        <?php else: ?>
        <div class="stic bg ic4 js-activity" id="js-status-object-<?= $aData['id'][$i]?>" data-id="<?= $aData['id'][$i]?>" data-status="0">
          <div class="sp">
            <div class="p">
              Chọn để kích hoạt
            </div>
...
if(!confirm("<?= Core::getPhrase('language_ban-co-chac-muon-xoa-bai')?>"))
 {
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=discount_item&val[status]=2&val[id]='+id);
...
if(!confirm("<?= Core::getPhrase('language_ban-co-chac-muon-xoa-danh-sach-bai')?>"))
 {
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=discount_item&val[status]=2&val[list]='+danh_sach);
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
...
function trang_thai_danh_sach_bai(trang_thai)
{
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=discount_item&val[status]='+trang_thai+'&val[list]='+danh_sach+'&val[math]='+Math.random());
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
...
function hien_thi(id, trang_thai) {
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=discount_item&val[status]='+trang_thai+'&val[id]='+id+'&val[math]='+Math.random());
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
