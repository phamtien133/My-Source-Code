<? if($this->_aVars['aData']['data']['status'] == 1) { ?>
    ...
    <section class="sanpham fthtab">
        ...
        <section class="product-list-box content-box panel-shadow mgbt20" ng-class="showlist">
            <div class="title_list">
                <div class="cl2 cl js-sort-by" data-link="<?= $this->_aVars['aData']['data']['link_sort']?>" data-sort='id_d'>
                    <div class="tt or">
                    Mã số
                    </div>
                ...
                <div class="cl3 cl js-sort-by" data-link="<?= $this->_aVars['aData']['data']['link_sort'].'&sort=1'?>" data-sort='name_d'>
                    <div class="tt or">
            ...
            <? endif?>
            <!-- Phân trang -->
            <?= Core::getService('core.tools')->paginate(ceil($this->_aVars['aData']['data']['total']/$this->_aVars['aData']['data']['page_size']), $this->_aVars['aData']['data']['page'], '/filter/general/?'.'&page=::PAGE::', '/filter/general/?', '', '')?>
...
if(!confirm("<?= Core::getPhrase('language_ban-co-chac-muon-xoa-danh-sach-bai')?>"))
 {
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.deleteObject&val[type]=filter_general&val[status]=2&val[list]='+danh_sach);
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
...
function trang_thai_danh_sach_bai(trang_thai)
{
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=filter_general&val[list]='+danh_sach+'&val[math]='+Math.random());
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
...
if(trang_thai == 0 && !confirm("<?= Core::getPhrase('language_ban-co-chac-khong-cho-phep-hien-thi-bai-viet')?>"))
{
 return false;
    ...
    http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=filter_general&val[status]='+trang_thai+'&val[id]='+stt+'&val[math]='+Math.random());
    http.onreadystatechange = function() {
        if(http.readyState == 4){
            var response = http.responseText;
            var error = http.responseText.split('<-errorvietspider->');
    ...
    $('.js-edit').each(function(){
       $(this).unbind('click').click(function(){
            ...
            if (typeof(id) == 'number' &&  id > 0) {
                links = '/filter/general/add/?id='+id;
