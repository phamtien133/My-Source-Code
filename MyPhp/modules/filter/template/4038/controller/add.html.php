    ...
    <div class="module mod-box">
        <div class="deepest">
            ...
            <form class="box style width100">
                <button onclick="window.location = '/filter/add/'" type="button"><span class="round"><span>
                <?= Core::getPhrase('language_them')?>
                </span></span></button>
                <button class="fright" type="button" onclick="window.location = '/filter/index/'"><span class="round"><span>
            ...
            <form method="post" name="frm_dang_ky" class="box style width100" onsubmit="return sbm_frm()">
                ...
                <table class="zebra">
                    <tr>
                        ...
                        <td>
                            <input class="inputbox" type="text" id="ten" name="ten" value="<?= $data['name']?>" />
                        </td>
                    </tr>
                    <tr>
                        <td><label for="ma_ten"><?= Core::getPhrase('language_ma-ten')?>:</label>(<a href="javascript:" onclick="return btn_cap_nhat_ma_ten()"><?= Core::getPhrase('language_cap-nhat-tu-dong')?></a>)<br>Lưu ý: Với hãng sản xuất, vui lòng đặt mã tên là <strong>production</strong></td>
                        <td>
                        <input class="inputbox" type="text" id="ma_ten" name="ma_ten" value="<?= $data['name_code']?>" onblur="kiem_tra_ma_ten()" />
                        </td>
                    </tr>
                    <tr>
                    ...
                    <select name="loai" id="loai" class="inputbox">
                        <?php for($i=0;$i<count($danh_sach_loai);$i++):?>
                        <option value="<?= $danh_sach_loai[$i]['ma_ten']?>"<?php if($data['type'] == $danh_sach_loai[$i]['ma_ten']) {?> selected="selected"<?php }?> title="<?= $danh_sach_loai[$i]['ghi_chu']?>"><?= $danh_sach_loai[$i]['ten']?></option>
                        <?php endfor?>
                    </select>
                        </td>
                    </tr>
                    <tr>
                        <td><label for="hien_thi">Show in:</label></td>
                        <td>
                    <input type="checkbox" value="1" name="hien_thi" id="hien_thi" class="inputbox"<?php if($data['is_display'] == 1) {?> checked<?php }?>> <?= Core::getPhrase('language_trang-chu')?>
                        </td>
                    </tr>
                    <tr>
...
<div id="dialog_search_box">
                <div style="background:#FFF;border:1px solid #CCC;padding-left:5px;">
                    ...
                    <option value="0"><?= Core::getPhrase('language_chon')?></option>
                    <? foreach($trich_loc as $v):?>
                        <option value="<?= $v['stt']?>"><?= $v['ten']?></option>
                    <? endforeach?>
                    </select>
                <div id="div_tim_bai_lien_ket_lien_quan"> </div>
            </div>
            <script>

var danh_sach_loai = [];
<?php foreach($trich_loc as $i => $val):?>
danh_sach_loai.push("<?= $val['loai']?>");
<?php endforeach?>
function get_list_tl(obj)
{
    var query = '&id=' + obj.val();
    var content = '', tiep = false;
    $.ajax({
    url: "/tools/acp/trich_loc_gia_tri.php?" + query,
    type: "GET",
    async:true,
    beforeSend: function ( xhr ) {
        $('#div_tim_bai_lien_ket_lien_quan').html('<?= Core::getPhrase('language_dang-tai-du-lieu')?>');
    },
    dataType: 'json',
    success: function (data) {
        ...
        if(data == null)
        {
            $('#div_tim_bai_lien_ket_lien_quan').html(content);
            return;
        }
        
        var trich_loc_vi_tri = -1;
        
        obj.find('option').each(function(index, element) {
            if($(this).attr('selected') != true) return true;
            
            trich_loc_vi_tri = index - 1;
            return false;
        });
        
        n = obj.val();
        
        var content = '<table class="zebra">';
        for(i = 0; i < data[n].length; i++)
        {
            // đưa danh sách stt đã chọn, nếu trùng, ẩn
            /*
            tmp = '';
            for(j = 0; j < gt_stt.length; j++)
            {
                if(gt_stt[j] == data[n][i]['stt'])
                {
                    tmp = ' checked="checked"';
                    break;
                }
            ...
            */
            ...
            content += '<tr><td><a href="javascript:">';
            if(typeof(danh_sach_loai[trich_loc_vi_tri]) != 'undefined' && danh_sach_loai[trich_loc_vi_tri].indexOf('-hinh-anh') != -1)
                content += '<img src="' + data[n][i]['ten'] + '" />';
            else
                content += data[n][i]['ten'];
            
            content += '</a><td><div style="display:none"><img src="http://img.<?= Core::getDomainName()?>/styles/acp/img/status_yes.png" title="<?= Core::getPhrase('language_da-chon')?>" /></div><div><a href="javascript:" onClick="return chon_trich_loc(' + data[n][i]['stt'] + ', \'' + data[n][i]['ten'] + '\')"><?= Core::getPhrase('language_chon')?></a></div></td></tr>';
        }
        content += '</table>';
        $('#div_tim_bai_lien_ket_lien_quan').html(content);
    }
...
function chon_trich_loc(stt, ten)
{
    var obj = $('#dialog_search_box #stt_bai_lien_ket_lien_quan').val();
    obj = '#' + obj;
    
    ten = ten + '-' + stt;
    if ($(obj).tagExist(ten)) return false;
    ...
    $('#dialog_search_box').modal(
        {
            onShow: function (dialog) {
                $('#dialog_search_box #stt_bai_lien_ket_lien_quan').val('cha_trich_loc_gt_' + slide);
...
if(is_array($data['gt_ten']))
{
    ...
    <?
    for($i=0;$i<count($data['gt_ten']);$i++)
    {
        $tmp_ten = str_replace("'", "\'", $data['gt_ten'][$i]);
        $tmp_ma_ten = str_replace("'", "\'", $data['gt_ma_ten'][$i]);
        $tmp_trang_thai = str_replace("'", "\'", $data['gt_trang_thai'][$i]);
        $tmp_stt = str_replace("'", "\'", $data['gt_stt'][$i]);
