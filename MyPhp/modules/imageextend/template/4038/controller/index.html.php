...
{
?>
<tr id="div_menu_<?= $shop_custom['stt'][$i]?>" class="div_menu div_menu_nhom_0">
    ...
    <td><a href="./imageextend/add/id_<?= $shop_custom['stt'][$i]?>"><?= $shop_custom['ten'][$i]?></a></td>
    ...
    <td><a href="./imageextend/add/id_<?= $shop_custom['stt'][$i]?>" title="<?= Core::getPhrase('language_sua-bai')?>"><img src="http://img.<?= Core::getDomainName()?>/styles/web/global/images/edit.png" alt="<?= Core::getPhrase('language_sua-bai')?>" /></a></td>
    ...
    <td align="center">
        <a href="javascript:" onclick="return tang_giam_vi_tri(0, <?= $shop_custom['stt'][$i]?>, 1)">
            <img src="http://img.<?= Core::getDomainName()?>/styles/acp/img/up.png" />
        </a>
        <a href="javascript:" onclick="return tang_giam_vi_tri(0, <?= $shop_custom['stt'][$i]?>, 0)">
            <img src="http://img.<?= Core::getDomainName()?>/styles/acp/img/down.png" />
        </a>
	</td>
    <td class="hidden val_pos_0"><?= $shop_custom['vi_tri'][$i]?></td>
</tr>
<?php
}
?>
...
<p class="buttonarea"><button type="button" onclick="window.location='./imageextend/add';"><span class="round"><span><?= Core::getPhrase('language_them')?></span></span></button></p>
...
if(!confirm("<?= Core::getPhrase('language_ban-co-chac-muon-xoa-bai')?>"))
 {
	...
	http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=image_extend&val[status]=2&val[id]='+stt);
...
if(!confirm("<?= Core::getPhrase('language_ban-co-chac-muon-xoa-danh-sach-bai')?>"))
 {
	...
	http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=image_extend&val[status]=2&val[list]='+danh_sach);
	http.onreadystatechange = function() {
		if(http.readyState == 4){
			var response = http.responseText;
			var error = http.responseText.split('<-errorvietspider->');
...
function trang_thai_danh_sach_bai(trang_thai)
{
	...
	http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=image_extend&val[status]='+trang_thai+'&val[list]='+danh_sach+'&val[math]='+Math.random());
	http.onreadystatechange = function() {
		if(http.readyState == 4){
			var response = http.responseText;
			var error = http.responseText.split('<-errorvietspider->');
...
if(trang_thai == 0 && !confirm("<?= Core::getPhrase('language_ban-co-chac-khong-cho-phep-hien-thi-bai-viet')?>"))
{
 return false;
	...
	http.open('get', '/includes/ajax.php?=&core[call]=core.updateStatus&val[type]=image_extend&val[status]='+trang_thai+'&val[id]='+stt+'&val[math]='+Math.random());
	http.onreadystatechange = function() {
		if(http.readyState == 4){
			var response = http.responseText;
			var error = http.responseText.split('<-errorvietspider->');
