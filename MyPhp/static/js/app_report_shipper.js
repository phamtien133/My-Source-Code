function initAppReport()
{
    ...
    $('#js-select-report-shipper').change(function(){
        var valSelect = $(this).val();
        if (valSelect == 'all') {
            $('#js-list-shipper-input').hide();
            return;
        }
        $('#js-list-shipper-input').show();
    });
    
    $('#js-view-report-shipper').click(function(){
        $('#frm_report_shipper').submit();
    });
    initPaginationReportShipper();
}

function initPaginationReportShipper()
{
    ...
    $('#js-pagination-report .item-trp-pagi').unbind('click').click(function(){
        ...
        //gọi ajax load page
        sParams = '&'+ getParam('sGlobalTokenName') + '[call]=app.getReportOrderShipper' + '&val[page]=' + iPage + '&val[type]=' + sType + '&val[date_begin]=' + iBegin + '&val[date_end]=' + iEnd + '&val[user_list]=' + iUserList;
        $.ajax({
            crossDomain:true,
            xhrFields: {
                ...
                if(isset(result.status) && result.status == 'success') {
                    ...
                    if (isset(result.data) && !empty(result.data)) {
                        for (var i in result.data) {
                            var sName = '';
                            if (!empty(result.data[i].shipper)) {
                                sName = result.data[i].shipper.fullname;
                            }
                            else {
                                sName = 'Không có T.T';
                            }
