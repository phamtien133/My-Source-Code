function initShoppers()
{
    ...
    $('.js-detail-user').click(function(){
       ...
       if (id > 0) {
           var sLink = '/app/shopper/detail/?id=' + id;
        ...
        insertPopupCrm('\
            <div class="container page-transport-popup pad20 js-popup-add-shipper" style="width: 500px">\
                <div class="content-box panel-shadow mgbt20">\
                    ...
                    <div class="box-inner">\
                        <form action="#" id="frm_add_shopper" method="post">\
                            <div class="row30">\
                            ...
                            <div class="row30 padtop">\
                                <div class="row30">\
                                    ...
                                    <div class="col3 padleft10">\
                                        <div class="button-default" id="js-cancel-add-shopper">Hủy</div>\
                                    </div>\
                                    <div class="col3 padleft10">\
                                        <input type="hidden" name="val[job_type]" value="0">\
                                        <div class="button-blue" id="js-submit-add-shopper">Thêm</div>\
                                    </div>\
                                </div>\
                            </div>\
                        </form>\
            ...
            </div>', ['#js-cancel-add-shopper'],'.js-popup-add-shopper', true);
            initAddShopper();
    });
}

function initAddShopper()
{
    $('#js-submit-add-shopper').click(function(){
        var obj = $(this);
        //submint form
        $(this).addClass('js-button-wait');
        var action = "$(this).ajaxSiteCall('app.register', 'afterRegisterShopper(data)'); return false;";
        $('#frm_add_shopper').attr('onsubmit', action);
        $('#frm_add_shopper').submit();
        $('#frm_add_shopper').removeAttr('onsubmit');
        return false; 
    });
}

function afterRegisterShopper(data)
{
    if (typeof(data.status) != 'undefined' && data.status == 'success') {
        alert('Thao tác thành công');
        $('#js-cancel-add-shopper').click();
        //refresh page
    ...
    $('#js-submit-add-shopper').removeClass('js-button-wait');
    return false;
}
...
function deleteShopper(id)
{
    ...
    sParams = '&'+ getParam('sGlobalTokenName') + '[call]=app.updateStatus' + '&val[type]=shopper' + '&val[status]=2&val[id]='+ id;
    $.ajax({
        crossDomain:true,
        xhrFields: {
...
function banShopper(id, status)
{
    ...
    sParams = '&'+ getParam('sGlobalTokenName') + '[call]=app.banUser' + '&val[type]=shopper' + '&val[status]=' + status +'&val[id]='+ id;
    $.ajax({
        crossDomain:true,
        xhrFields: {
            withCredentials: true
